...
- **注入字段错位（第十层，最终根因）**：x64 `INPUT` 的 union 含 8 字节对齐指针成员 → union 对齐 8，`type@0` 后有 **4 字节 pad**，`KEYBDINPUT` 实际位于 `wVk@8, wScan@10, dwFlags@12`。显式布局曾把 `wVk@4, wScan@6` 写进 pad 区——字节转储对比确认错位；`SendInput` 按 40 字节校验通过、读到 `wVk=0` 的**空键事件**返回 1，前台收不到任何有效按键（"成功却无效"）。全部字段偏移 +4 修复（`wVk@8, wScan@10, kbFlags@12, time@16, dwExtraInfo@20`；MOUSEINPUT 同步）。
- **注入形态（第九层）**：scan-only 形态（`wVk=0, KEYEVENTF_SCANCODE`）会被微信输入法按 VK 匹配的钩子过滤。决定性实验：独立 SendInput（40 字节、`wVk+wScan` 双信息、无 SCANCODE 标志）注入 Ctrl+Win 3 秒 → 微信语音条弹出（用户确认）；手动 Ctrl+Win 同样有效。最终采用 VK+scan 双信息形态。
- **真机验证结论（2026-09-08 23:0x）**：按住遥控器语音键 → F5 被钩子吞掉（无浏览器刷新）→ `HID VOICEKEY down` → `INJECT voice mode=CtrlWinHold ok=True` → 微信输入法语音条弹出 → 说话 → 松开语音键出字。F5 泄漏与微信触发两条链路均闭环。物理键盘 F5 失效（Ctrl+R 替代刷新）为已确认取舍；方向/OK/Back/Home/Menu 保持系统原生行为。
