...
| 模式 | 行为 | 适用 |
| --- | --- | --- |
| **Fn/语音键（按住说话，默认）** | 按住说话期间保持 F5 键按下，松开释放 | 直接把语音送给听写/输入工具 |
| **左 Command 长按**（Windows = 左 Ctrl） | 按住语音键 = 按住左 Ctrl | 目标应用以按住 Ctrl 触发录音 |
| **右 Command 长按**（Windows = 右 Ctrl） | 同上，右 Ctrl | 同上 |
| **Fn 点按** | 开始/结束各点按一次 F5；语音开头最多缓存 0.5 秒补写 | 配合使用方的点按式语音工具（需把触发键设为 F5） |
...
- **Fn 点按语义**：注入的是 F5 键，不是物理 Fn 键；需在使用方的语音工具中把触发键设为 F5。
...
- fn（语音键）→ 注入 F5：普通模式按住说话期间保持 F5 按下；「Fn 点按」模式改为开始/结束各一次点按，点按模式下语音开头 pre-roll 最多缓存 0.5 秒，点按完成后写入
